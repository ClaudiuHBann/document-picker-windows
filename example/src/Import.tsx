...
export function ImportExamples() {
  ...
  return (
    <View style={styles.section} testID={'importExamples'}>
        ...
        <Button
          ...
          onPress={async () => {
            try {
              ...
              const filesToCopy: Array<FileToCopy> = pickResults.map(
                (file: any) => ({
                  uri: file.uri,
              ...
              const localCopyResults: LocalCopyResponse[] = copyResults.map(
                (it) => {
                  invariant(
                    it.status === 'success',
        ...
        <Button
          ...
          onPress={async () => {
            try {
              ...
              const getLocalUri = Platform.select({
                android: async () => {
                  const virtualFileMeta =
                    convertibleToMimeTypes && convertibleToMimeTypes[0];
                  invariant(
              ...
              addResult([
                { status: 'success', sourceUri: pickedUri, localUri },
              ]);
            } catch (err) {
              handleError(err);
            }
