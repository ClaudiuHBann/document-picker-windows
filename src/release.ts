...
export const releaseLongTermAccess = async (
  ...
  return NativeDocumentPickerWindows.releaseLongTermAccess(
    uris
  ) as Promise<ReleaseLongTermAccessResult>;
};
...
export const releaseSecureAccess = async (uris: string[]): Promise<null> => {
  return NativeDocumentPickerWindows.releaseSecureAccess(uris);
};
