import NativeDocumentPickerWindows, {
  type LocalCopyResponse,
} from './spec/NativeDocumentPickerWindows';
 ...
 * */
export function keepLocalCopy(
  options: KeepLocalCopyOptions
): Promise<KeepLocalCopyResponse> {
  const response = NativeDocumentPickerWindows.keepLocalCopy(options);
