 ...
 * @group pick() types
 */
export type DocumentPickerResponseOpenLongTerm = DocumentPickerResponse &
  BookmarkingResponse;
...
type DoPickOptions = DocumentPickerOptionsBase & {
  mode: 'import' | 'open';
  type: string[];
};
 ...
 * */
export async function pick<O extends DocumentPickerOptions>(
  options?: O
): PickResponse<O> {
  const type: string[] = (() => {
    if (!options?.type) {
  ...
  const response: Promise<DocumentPickerResponse[]> =
